...
Run: pytest test_ml_forecast_service.py -v
Place this file in the same folder as ml_forecast_service.py
"""
...
# ── Locate ml_forecast_service.py next to this test file ─────────────────────
_HERE = Path(__file__).parent
_MODULE_PATH = _HERE / "ml_forecast_service.py"

if not _MODULE_PATH.exists():
    pytest.exit(
        f"\n\nCannot find ml_forecast_service.py in:\n  {_HERE}\n"
        "Place this test file in the same directory as the source file.\n",
        returncode=1,
    )

# Load the module once for the whole session
_spec   = importlib.util.spec_from_file_location("ml_forecast_service", _MODULE_PATH)
_ml_mod = importlib.util.module_from_spec(_spec)
sys.modules.setdefault("ml_forecast_service", _ml_mod)
